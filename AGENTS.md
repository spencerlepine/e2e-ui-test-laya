...
- The text model plans once per task (requirements, the item to open, the finish condition, or ordered steps). In Laya
  mode TYPE_TEXT types a planned value; it does not call the text model per field. In steps mode, when no visible
  label names a step, the text model may pick one observed control by index (cached, at most 3 calls per step);
  any failure means "none" and the step waits.
...
- Keep credentials server-side. `.env` is committed with local-only values; never put real keys in it. Tests must not call paid APIs.
